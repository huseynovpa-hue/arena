...
export function formatDate(dateStr, t) {
  ...
  if (d.getTime() < now.getTime()) day = t?.started || "Started";
    ...
    if (lang === "az") {
      ...
      const azDays = ["B.", "B.e.", "Ç.a.", "Ç.", "C.a.", "C.", "Ş."];
      const bd = new Date(d.toLocaleString("en-US", { timeZone: TIMEZONE }));
      day = `${azDays[bd.getDay()]} ${bd.getDate()} ${azMonths[bd.getMonth()]}`;
    } else {
      day = d.toLocaleDateString("en-US", { timeZone: TIMEZONE, weekday: "short", month: "short", day: "numeric" });
    }
  }
