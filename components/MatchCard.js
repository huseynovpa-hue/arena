  ...
  return (
    <div className={`card transition-colors ${locked ? "border-green-500/25" : ""}`}>
      ...
      <div className="p-3.5">
        ...
        {/* Status / Lock */}
        {statusLabel && (
          <div className={`mt-3 py-2 rounded-xl text-center border text-[11px] font-bold tracking-wide ${statusLabel.bg} ${statusLabel.color}`}>
            {statusLabel.text}
          </div>
        )}
        ...
        {locked && !statusLabel && (
          <div className="mt-3 py-2 rounded-xl text-center bg-green-500/10 border border-green-500/25 text-green-400 text-[11px] font-bold tracking-wide">
            ✓ LOCKED — {homeScore} : {awayScore} · First: {firstToScore === "home" ? match.home_team : firstToScore === "away" ? match.away_team : "No Goal"}
          </div>
        )}
      </div>
    </div>
