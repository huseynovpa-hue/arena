...
export default function LeaderboardPage() {
  ...
  async function loadLeaderboard() {
    ...
    if (filter === "all") {
      ...
      setPlayers(data || []); setNoResults(null); setLoading(false); return;
    }
    ...
    if (filter === "month") {
      ...
      const { data: matches } = await supabase.from("matches").select("id, status").gte("kick_off", startDate).lte("kick_off", endDate);
      matchIds = (matches || []).map(m => m.id);
      hasFinished = (matches || []).some(m => m.status === "finished");
    } else if (filter === "week" && selectedWeek) {
      const { data: matches } = await supabase.from("matches").select("id, status").eq("week_id", selectedWeek);
      matchIds = (matches || []).map(m => m.id);
      hasFinished = (matches || []).some(m => m.status === "finished");
    }

    if (matchIds.length === 0) { setPlayers([]); setNoResults("none"); setLoading(false); return; }
    if (!hasFinished) { setPlayers([]); setNoResults("pending"); setLoading(false); return; }
    setNoResults(null);

    const { data: preds } = await supabase.from("predictions").select("user_id, points").in("match_id", matchIds).not("points", "is", null);
    const userMap = {};
    (preds || []).forEach(p => {
    ...
    const result = allProfiles.map(p => ({
      ...
      accuracy: userMap[p.id]?.count > 0 ? ((userMap[p.id].total / (userMap[p.id].count * 50)) * 100).toFixed(1) : "0.0",
      perfect_scores: 0,
  ...
  return (
    <div className="py-4">
      <h1 className="text-lg font-black mb-4">🏆 {t.leaderboard}</h1>

      <div className="flex gap-1.5 mb-3">
        {[{ k: "all", l: t.allTime, ac: "green" }, { k: "week", l: t.weekly, ac: "blue" }, { k: "month", l: t.monthly, ac: "amber" }].map(f => (
          <button key={f.k} onClick={() => setFilter(f.k)}
            className={`px-4 py-2 rounded-full text-xs font-bold border transition-all ${
              f.k === filter ? `bg-${f.ac}-500/15 border-${f.ac}-500 text-${f.ac}-400` : "border-[--border] text-[--muted]"
            }`}>{f.l}</button>
        ))}
      </div>
      ...
      {filter === "month" && (
        <div className="mb-3">
          <select value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="input-dark w-auto pr-8 text-xs cursor-pointer" style={{ maxWidth: 200 }}>
      ...
      {filter === "week" && (
        <div className="mb-3">
          {weeks.length === 0 ? <p className="text-xs text-[--muted]">{t.noMatchesWeek}</p> : (
      ...
      {loading ? (
        ...
        <div className="card p-12 text-center">
          ...
          <h3 className="text-lg font-bold mb-1.5">{noResults === "pending" ? t.resultsOnTheWay : t.noMatchesFound}</h3>
          <p className="text-xs text-[--muted]">
            {noResults === "pending" ? t.standingsAvailable : filter === "month" ? t.noMatchesMonth : filter === "week" ? t.noMatchesWeek : t.playersAppear}
          </p>
        </div>
      ) : (
        <div className="card overflow-hidden">
          <div className="flex items-center px-4 py-2.5 bg-[--surface] border-b border-[--border] text-[9px] text-[--muted] uppercase tracking-wider font-bold">
            ...
            {filter === "month" && <span className="w-16 text-right">{t.prize}</span>}
            <span className="w-16 text-right">{t.preds}</span>
            <span className="w-16 text-right">{t.acc}</span>
            <span className="w-20 text-right">{t.points}</span>
          </div>
          {players.map((p, i) => (
            <div key={p.id} className={`flex items-center px-4 py-2.5 border-b border-[--border] last:border-0 ${i < 3 ? rankBg[i] || "" : ""}`}>
              ...
              {filter === "month" && (
                <span className="w-16 text-right">
                  {i < 3 && p.total_points > 0 ? <span className={`text-[11px] font-bold ${prizeColors[i]}`}>{prizeLabels[i]}</span> : <span className="text-[11px] text-[--muted]">—</span>}
                </span>
              )}
